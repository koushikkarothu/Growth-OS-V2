...
export default function AICoachPage() {
  ...
  async function autoFillWord() {
      ...
      try {
          const res = await fetch('/api/ai-tutor', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ prompt: customPrompt, expectJson: langMode === 'de' }) })
          if (!res.ok) throw new Error("API Failed");
          const data = await res.json()
          
          if (data.result) {
              if (langMode === 'en') setTranslation(data.result);
              else {
    ...
    <div className="w-full pb-32 animate-in fade-in duration-500 relative">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 mt-2 border-b border-slate-200 dark:border-slate-800 pb-8">
        ...
        <div className="flex items-center gap-3">
           <button onClick={() => {fetchGlobalNotes(); setIsNotesDrawerOpen(true);}} className="flex items-center gap-2 px-4 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 font-bold rounded-lg shadow-sm hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors text-sm">
               <Library size={16} /> Notes Library
           </button>
      ...
      {activeTab === 'vault' && (
        <div className="animate-in slide-in-from-left-4">
          ...
          {isAdding && (
            <form onSubmit={addWord} className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xl mb-10 grid grid-cols-1 md:grid-cols-2 gap-5 animate-in slide-in-from-top-4">
              <div className="md:col-span-2 relative">
                <label className="text-xs font-bold text-slate-500 uppercase tracking-widest block mb-1.5">{langMode === 'en' ? 'Word / Phrase' : 'German Word'}</label>
              ...
              {langMode === 'de' && (
                  <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-5 border-t border-slate-100 dark:border-slate-800 pt-5 mt-1">
                    <div><label className="text-xs font-bold text-slate-500 uppercase tracking-widest block mb-1.5">Word Type</label><select value={wordType} onChange={e => setWordType(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-3 text-sm font-semibold outline-none dark:text-white"><option>Noun</option><option>Verb</option><option>Adjective</option><option>Adverb</option><option>Other</option></select></div>
              ...
              <div><label className="text-xs font-bold text-slate-500 uppercase tracking-widest block mb-1.5">{langMode === 'en' ? 'Definition' : 'English Translation'}</label><input type="text" value={translation} onChange={e => setTranslation(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-3 text-sm font-semibold outline-none dark:text-white shadow-sm" required /></div>
              {langMode === 'de' && wordType === 'Noun' && (<div><label className="text-xs font-bold text-slate-500 uppercase tracking-widest block mb-1.5">Plural Form</label><input type="text" placeholder="e.g. die Autos" value={plural} onChange={e => setPlural(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-3 text-sm font-medium outline-none dark:text-white shadow-sm" /></div>)}
              {langMode === 'de' && wordType === 'Verb' && (<div className="md:col-span-2"><label className="text-xs font-bold text-slate-500 uppercase tracking-widest block mb-1.5">Conjugation / Notes</label><input type="text" placeholder="e.g. ich fahre, du fährst / Perfekt: bin gefahren" value={conjugation} onChange={e => setConjugation(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-3 text-sm font-medium outline-none dark:text-white shadow-sm" /></div>)}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 relative">
            {filteredVocab.map(v => (
              <div key={v.id} className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm relative group hover:shadow-md hover:border-indigo-300 dark:hover:border-slate-700 transition-all flex flex-col">
                 <div className="flex items-start justify-between mb-4">
                     {langMode === 'de' ? (<span className={cn("text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded", v.word_type === 'Noun' ? "bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400" : v.word_type === 'Verb' ? "bg-emerald-50 text-emerald-600 dark:bg-emerald-900/20 dark:text-emerald-400" : "bg-purple-50 text-purple-600 dark:bg-purple-900/20 dark:text-purple-400")}>{v.word_type}</span>) : <div />}
                     <div className="flex items-center gap-1 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                        <button onClick={() => setEditingWord(v)} className="text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:text-indigo-400 dark:hover:bg-indigo-500/10 transition-all p-1.5 rounded-md" title="Edit Data"><Pencil size={14} /></button>
                        <button onClick={() => deleteWord(v.id)} className="text-slate-400 hover:text-red-600 hover:bg-red-50 dark:hover:text-red-400 dark:hover:bg-red-500/10 transition-all p-1.5 rounded-md" title="Delete"><Trash2 size={14} /></button>
                     </div>
                 </div>
                 <div className="flex items-start justify-between gap-3">
                     <div className="min-w-0 pr-2">
                         ...
                         <p className="text-slate-500 dark:text-slate-400 font-medium text-sm leading-snug line-clamp-2">{v.translation || v.definition}</p>
                     </div>
                     <button onClick={() => playAudio(v.word)} title="Listen to Pronunciation" className="shrink-0 w-10 h-10 bg-slate-50 hover:bg-indigo-50 dark:bg-slate-800 dark:hover:bg-indigo-500/20 text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 rounded-full flex items-center justify-center transition-all border border-slate-200 dark:border-slate-700"><Volume2 size={16} className="ml-0.5" /></button>
                 </div>
                 {(v.plural_form || v.conjugation) && (
                   <div className="mt-auto pt-4">
                     <div className="p-3 bg-slate-50 dark:bg-slate-950 rounded-xl text-xs font-medium text-slate-600 dark:text-slate-400 border border-slate-100 dark:border-slate-800">
                       {v.plural_form && <div><span className="text-slate-400 font-bold mr-2 uppercase tracking-widest text-[9px]">Plural:</span>{v.plural_form}</div>}
                       {v.conjugation && <div className={v.plural_form ? "mt-1.5" : ""}><span className="text-slate-400 font-bold mr-2 uppercase tracking-widest text-[9px]">Conj:</span>{v.conjugation}</div>}
                     </div>
                   </div>
                 )}
              </div>
          ...
          {editingWord && (
             <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in">
                <div className="bg-white dark:bg-slate-900 w-full max-w-xl p-6 md:p-8 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xl relative">
                    ...
                    <form onSubmit={handleUpdateWord} className="space-y-4">
                        {langMode === 'de' && (
                          <div className="grid grid-cols-2 gap-4">
                              <div><label className="text-xs font-bold text-slate-500 uppercase tracking-widest block mb-1">Word Type</label><select value={editingWord.word_type || 'Noun'} onChange={e => setEditingWord({...editingWord, word_type: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg px-3 py-2.5 text-sm font-semibold outline-none dark:text-white"><option>Noun</option><option>Verb</option><option>Adjective</option><option>Adverb</option><option>Other</option></select></div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-8 space-y-6">
               <div className="bg-slate-50 dark:bg-slate-900/50 p-6 md:p-8 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
                 <div className="flex items-center gap-3 mb-4"><div className="w-8 h-8 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded-lg flex items-center justify-center border border-indigo-200 dark:border-indigo-800"><Bot size={16}/></div><span className="font-bold text-slate-900 dark:text-slate-200 tracking-wider uppercase text-xs">Professor Gemini</span></div>
                 <p className="text-base font-medium text-slate-700 dark:text-slate-300 leading-relaxed whitespace-pre-wrap">{drillScenario || "Select your target vocabulary from the left and initiate the drill."}</p>
               </div>
               {isDrilling && drillScenario && (
                 <div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm animate-in fade-in slide-in-from-bottom-4">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-3 block">Construct Your Sentence</label>
                    <textarea value={userSentence} onChange={e => setUserSentence(e.target.value)} placeholder={langMode === 'en' ? "Type your English sentence here..." : "Type your German sentence here..."} className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-4 text-sm font-medium outline-none focus:border-indigo-500 dark:text-white resize-none h-32 mb-4" />
                    <button onClick={gradeSentence} disabled={!userSentence} className="w-full md:w-auto bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-8 py-2.5 rounded-lg font-bold shadow-sm flex items-center justify-center gap-2 disabled:opacity-50 transition-all text-sm"><Send size={16} /> Submit for Grading</button>
                 </div>
               )}
               {feedback && (
                 <div className="bg-emerald-50 dark:bg-emerald-900/10 p-6 md:p-8 rounded-2xl border border-emerald-200 dark:border-emerald-900/30 shadow-sm animate-in zoom-in-95">
                    <h3 className="text-lg font-black text-emerald-800 dark:text-emerald-400 flex items-center gap-2 mb-4"><CheckCircle2 className="text-emerald-500" size={20} /> Evaluation Report</h3>
                    <div className="prose dark:prose-invert max-w-none text-sm font-medium leading-relaxed text-slate-700 dark:text-slate-300">
